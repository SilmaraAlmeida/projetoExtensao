...
$dados = $dados['dados'];
?>
<div class="min-h-screen bg-gray-50" x-data="perfilUsuario()">
    <!-- Header do Dashboard -->
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <form action="<?= baseUrl() ?>Sistema/atualizarPerfil" method="POST" class="space-y-8">

            <?php if ($dados['tipo'] === 'A'): ?>
                <!-- ============================================ -->
                <!-- FORMULÁRIO PARA EMPRESA (TIPO A) -->
                <!-- ============================================ -->
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input
                                    ...
                                    value="<?= $dados['nome'] ?? '' ?>"
                                    :disabled="!modoEdicao"
                                    class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg text-base text-gray-700 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-200 transition-all duration-200 disabled:bg-gray-100 disabled:cursor-not-allowed"
                            ...
                            <div>
                                <label for="latitude" class="block text-sm font-semibold text-gray-700 mb-2">
                                    <i class="fas fa-location-arrow text-gray-400 mr-2"></i>
                                    Latitude
                                </label>
                                <input
                                    type="text"
            ...
            <?php else: ?>
                ...
                <!-- ============================================ -->
                <!-- FORMULÁRIO PARA PESSOA FÍSICA (TIPO CN/G) -->
                <!-- ============================================ -->
            ...
            <!-- Card de Telefones (COMUM - EDITÁVEL) -->
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="p-6">
                    ...
                    <div x-show="telefones.length === 0"
                        ...
                        <div class="text-center py-8 text-gray-500">
                            ...
                            <p class="text-sm" x-show="modoEdicao">Clique em "Adicionar Telefone" para incluir seus contatos.</p>
                        </div>
                    </div>

                    <!-- Lista de telefones -->
                    <div class="space-y-4" x-show="telefones.length > 0">
                        <template x-for="(telefone, index) in telefones" :key="telefone.telefone_id || 'novo-' + index">
                            <div class="bg-gray-50 p-4 rounded-lg border border-gray-200 hover:shadow-sm transition-shadow duration-200"
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                                    ...
                                    <!-- Número -->
                                    <div :class="modoEdicao ? 'md:col-span-7' : 'md:col-span-9'">
                                        ...
                                        <input
                                            ...
                                            :disabled="!modoEdicao"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-200 transition-all duration-200 disabled:bg-gray-100 disabled:cursor-not-allowed"
                                            placeholder="(32) 99999-9999"
                                            maxlength="15">
                                        <input type="hidden" :name="'telefones[' + index + '][telefone_id]'" x-model="telefone.telefone_id">
                                    </div>
                                    ...
                                    <!-- Remover -->
                                    <div class="md:col-span-2" x-show="modoEdicao">
                                        <button
                                            type="button"
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-end">
                ...
                <!-- Botões Cancelar/Salvar (modo edição) -->
                <template x-if="modoEdicao">
                    <div class="flex gap-4">
                        <button
                            type="button"
                            @click="cancelarEdicao()"
                            class="inline-flex items-center justify-center px-3 py-2 border border-red-400 text-red-600 rounded-lg text-base font-semibold hover:border-red-500 hover:text-red-700 hover:bg-red-100 focus:outline-none focus:ring-4 focus:ring-red-200 transition-all duration-200">
                            <i class="fas fa-times mr-2"></i>
                            Cancelar
                        </button>
                        <button
                            type="submit"
                            class="inline-flex items-center justify-center px-3 py-2 border border-gray-300 text-gray-700 rounded-lg text-base font-semibold hover:border-gray-400 hover:bg-gray-50 focus:outline-none focus:ring-4 focus:ring-gray-200 transition-all duration-200">
...
<script>
    function perfilUsuario() {
        return {
            modoEdicao: false,
            cpf: '<?= $dados['cpf'] ?? '' ?>',
            telefones: <?= json_encode($dados['telefones'] ?? []) ?>,
